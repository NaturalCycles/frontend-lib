...
import type ky from './index'

const globals = {} as any

const getGlobal = (property: any) => {
...
const deepMerge = (...sources: any[]) => {
  ...
  for (const source of sources) {
    if (Array.isArray(source)) {
      ...
      returnValue = [...(returnValue as any), ...source]
...
const timeout = (promise: any, ms: any, abortController: any) =>
...
const normalizeRequestMethod = (input: any) =>
...
const normalizeRetryOptions = (retry: any = {}) => {
...
class Ky {
  ...
  constructor(input: any, options: any = {}) {
    ...
    if (
      ...
      !(this._input instanceof URL || this._input instanceof globals.Request)
    ) {
    ...
    if (this._options.searchParams) {
      const url = new URL(this.request.url) as any
      url.search = new URLSearchParams(this._options.searchParams)

      // To provide correct form boundary, Content-Type header should be deleted each time when new Request instantiated from another one
      if (
        ((supportsFormData && this._options.body instanceof globals.FormData) ||
  ...
  _calculateRetryDelay(error: any) {
    ...
    if (this._retryCount < this._options.retry.limit && !(error instanceof TimeoutError)) {
      if (error instanceof HTTPError) {
        if (!this._options.retry.statusCodes.includes((error as any).response.status)) {
          return 0
        }
        ...
        if (
          retryAfter &&
          this._options.retry.afterStatusCodes.includes((error as any).response.status)
        ) {
...
const createInstance = (defaults?: any) => {
  ...
  for (const method of requestMethods) {
    ky[method] = (input: any, options: any) =>
...
const kyLib = (createInstance() as any) as typeof ky
